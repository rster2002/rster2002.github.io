...
async function addTolist(i, characterId, delay) {
	userRef.collection("characters").doc(characterId).get().then(function(info) {
		if (info && info.exists) {
			var characterInfo = info.data();
			console.log("info");

			userRef.collection("characters").doc(characterId + "/data/characterObj").get().then(function(o) {
				console.log(o);
