// https://mcapi.us/server/status?ip=149.202.90.3&port=41070
...
function updateInfo() {
    fetch("https://mcapi.us/server/status?ip=149.202.90.3&port=41070")
        ...
        .then(json => {
            ...
            if (json.online) {
                document.getElementById("status").innerHTML = "Players online: " + json.players.now + "/" + json.players.max;
            } else {
                document.getElementById("status").innerHTML = "The server is currently offline";
            }
        });
}
