			...
			<div class="card">
				...
				<div class="iconList" v-if="selected.selected">
					<icon-text icon="account" v-if="selected.assignee.length !== 0">
						<p><span v-for="assignee in selected.assignee" class="chip"><img :src="assignee.avatar_url" />{{ assignee.login }}</span></p>
					</icon-text>
					<icon-text icon="label" v-if="selected.labels.length !== 0">
						<p>
							<span v-for="label in selected.labels" class="label" :style="{ backgroundColor: '#' + label.color, color: c(label.color) }">{{ label.name }}</span>
						</p>
					</icon-text>
